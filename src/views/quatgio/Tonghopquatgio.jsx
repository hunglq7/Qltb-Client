...
import { useTonghopquatgioStore } from '../../stores/quatgio/tonghopquatgioStore';
import NhatkyquatgioTable from '../../sections/quatgio/NhatkyquatgioTable';
import ThongsoquatgioTable from '../../sections/quatgio/ThongsoquatgioTable';
import TonghopquatgioForm from '../../sections/quatgio/TonghopquatgioForm';
import * as XLSX from 'xlsx';
import dayjs from 'dayjs';
import ActionBar from '/src/components/ActionBar';
import SearchBar from '/src/components/SearchBar';

const Tonghopquatgio = () => {
  const [activeTab, setActiveTab] = useState('1');
  const [selectedRowKeys, setSelectedRowKeys] = useState([]);
  const [quatGio, setQuatGio] = useState([]);
  ...
  const {
    ...
    createTonghopquatgio,
    updateTonghopquatgio,
    deleteTonghopquatgio,
    deleteMultipleTonghopquatgio,
    totalRecords
  } = useTonghopquatgioStore();
  ...
  const handleDeleteMultiple = async () => {
    try {
      await deleteMultipleTonghopquatgio(selectedRowKeys);
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (editing) {
        await updateTonghopquatgio(editing.id, payload);
        message.success('Cập nhật thành công');
      } else {
        await createTonghopquatgio(payload);
  ...
  const handleExportExcel = () => {
    const exportData = dataTonghopquatgio.map((item, index) => ({
      STT: index + 1,
    ...
    XLSX.utils.book_append_sheet(workbook, worksheet, 'Capnhatquatgio');
    XLSX.writeFile(workbook, `Cap-Nhat-Quat-Gio-${dayjs(new Date()).format('DD-MM-YYYY')}.xlsx`);
  };
  ...
  const tabItems = [
    {
      ...
      children: (
        <TonghopquatgioForm
          ...
          quatGioList={dataDanhmucquatgio}
          donViList={dataDonvi}
        />
      )
    },
    {
      ...
      disabled: !editing,
      children: editing ? <NhatkyquatgioTable tonghopquatgio={quatGio} /> : <div>Chọn bản ghi để xem nhật ký thiết bị</div>
    },
    {
      key: '3',
      label: 'THÔNG SỐ KỸ THUẬT',
      disabled: !editing,
      children: editing ? <ThongsoquatgioTable tonghopquatgio={quatGio} /> : <div>Chọn bản ghi để xem thông số kỹ thuật</div>
    }
  ];
  return (
    <MainCard>
