...
import { useNhatkybomnuocStore } from '../../stores/bomnuoc/nhatkybomnuocStore';
...
const NhatkybomnuocTable = ({ tonghopbomnuoc }) => {
  const [form] = Form.useForm();
  const [editingKey, setEditingKey] = useState(null);
  const [localData, setLocalData] = useState([]);
  const [selectedRowKeys, setSelectedRowKeys] = useState([]);
  const [searchText, setSearchText] = useState('');
  const tonghopbomnuocId = tonghopbomnuoc?.id;
  const {
    dataNhatkybomnuoc,
    loading,
    getNhatkybomnuocById,
    createNhatkybomnuoc,
    updateNhatkybomnuoc,
    deleteNhatkybomnuoc,
    deleteMultiple
  } = useNhatkybomnuocStore();

  // ================= LOAD DATA =================
  useEffect(() => {
    if (tonghopbomnuocId) getNhatkybomnuocById(tonghopbomnuocId);
  }, [tonghopbomnuocId]);
  ...
  const dataSource = useMemo(() => {
    return [
      ...localData,
      ...dataNhatkybomnuoc.map((x) => ({
        ...x,
        key: x.id
      }))
    ];
  }, [localData, dataNhatkybomnuoc]);
  const isEditing = (record) => record.key === editingKey;
  console.log(tonghopbomnuoc.id);
  ...
  const handleAdd = () => {
    ...
    const newRow = {
      key,
      id: null,
      tonghopbomnuocId: tonghopbomnuocId,
      ngaythang: dayjs(),
      donVi: '',
      viTri: '',
      trangThai: true,
  ...
  const save = async (key) => {
    try {
      ...
      if (String(key).startsWith('new_')) {
        await createNhatkybomnuoc(createPayload);
        message.success('Thêm mới thành công');
      } else {
        await updateNhatkybomnuoc(id, payload);
        message.success('Cập nhật thành công');
      }
      getNhatkybomnuocById(tonghopbomnuocId);
      cancel();
    } catch (err) {
      console.error(err);
  ...
  const handleDelete = async (record) => {
    ...
    await deleteNhatkybomnuoc(record.id);
...
export default memo(NhatkybomnuocTable);
