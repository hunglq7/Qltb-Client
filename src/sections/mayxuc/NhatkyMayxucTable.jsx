...
function NhatkyMayxucTable({ thongsomayxuc }) {
  ...
  const tonghopmayxucId = thongsomayxuc?.id;
  ...
  // ================= LOAD DATA =================
  useEffect(() => {
    if (tonghopmayxucId) getNhatkymayxucById(tonghopmayxucId);
  }, [tonghopmayxucId]);
  ...
  const dataSource = useMemo(() => {
    return [
      ...localData,
      ...dataNhatkyMayxuc.map((x) => ({
        ...x,
        key: x.id
      }))
    ];
  }, [localData, dataNhatkyMayxuc]);
  ...
  const save = async (key) => {
    try {
      ...
      cancel();
      getNhatkymayxucById(tonghopmayxucId);
    } catch (err) {
      console.error(err);
