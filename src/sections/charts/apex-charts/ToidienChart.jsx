...
const ToidienChart = () => {
  const { dataTonghop, loading, fetchTonghoptoidien } = useTonghoptoidienStore();
  useEffect(() => {
    fetchTonghoptoidien();
  }, []);
  // ✅ LUÔN ÉP DATA VỀ ARRAY
  const listData = useMemo(() => (Array.isArray(dataTonghop) ? dataTonghop : dataTonghop?.items || []), [dataTonghop]);
